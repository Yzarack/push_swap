...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/05/24 13:09:43 by jthierce          #+#    #+#             */
/*   Updated: 2019/07/05 11:19:04 by jthierce         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

unsigned long long	ft_hexad_conver(void *nb, t_flags options)
{
...
char				*ft_hexad_flags(char *str, t_flags options, int spreci,
...
int					ft_hexad_speprint(t_flags options, unsigned long long inb)
{
...
int					ft_hexa_down(void *nb, t_flags options)
{
	...
	options.sign = 0;
	options.space = 0;
	inb = ft_hexad_conver(nb, options);
	...
	if (spreci == -1 && options.zero > ucount_min_preci(inb, 16))
		options.preci = ft_hexad_speprint(options, inb);
	str = ft_printf_utoa_base(inb, "0123456789abcdef", options, '0');
	str = ft_hexad_flags(str, options, spreci, inb);
